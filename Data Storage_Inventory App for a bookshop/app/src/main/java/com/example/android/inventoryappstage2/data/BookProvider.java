...
public class BookProvider extends ContentProvider {
    ...
    private Uri insertBook(Uri uri, ContentValues values) {
        ...
        if (title == null) {
            throw new IllegalArgumentException(String.valueOf(R.string.book_requires_title));
        }

        // Check that the author is not null
        String author = values.getAsString(BookEntry.COLUMN_BOOK_AUTHOR);
        if (author == null) {
            throw new IllegalArgumentException(String.valueOf(R.string.book_requires_author));
        }

        // Check that the genre is valid
        String genre = values.getAsString(BookEntry.COLUMN_BOOK_GENRE);
        if (genre == null || !BookEntry.isValidGenre(genre)) {
            throw new IllegalArgumentException(String.valueOf(R.string.book_requires_genre));
        }

        // Check that the unit price is valid
        Double price = values.getAsDouble(BookEntry.COLUMN_BOOK_PRICE);
        if (price != null && price < 0) {
            throw new IllegalArgumentException(String.valueOf(R.string.book_requires_price));
        }

        // Check that the quantity of books in stock is valid
        Integer quantity = values.getAsInteger(BookEntry.COLUMN_BOOK_QUANTITY);
        if (quantity != null && quantity < 0) {
            throw new IllegalArgumentException(String.valueOf(R.string.book_requires_quantity));
        }

        // Check that the supplier's name is valid
        String supname = values.getAsString(BookEntry.COLUMN_BOOK_SUPNAME);
        if (author == null) {
            throw new IllegalArgumentException(String.valueOf(R.string.book_requires_supname));
        }

        // Check that the supplier's phone number is valid
        Integer supphone = values.getAsInteger(BookEntry.COLUMN_BOOK_SUPPHONE);
        if (price != null && price < 0) {
            throw new IllegalArgumentException(String.valueOf(R.string.book_requires_supphone));
        }
    ...
    private int updateBook(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        if (values.containsKey(BookEntry.COLUMN_BOOK_QUANTITY)) {
            // Check that the stock quantity is greater than or equal to 0
            Integer quantity = values.getAsInteger(BookEntry.COLUMN_BOOK_QUANTITY);
            if (quantity != null && quantity < 0) {
                throw new IllegalArgumentException(String.valueOf(R.string.book_requires_quantity));
            }
        ...
        // If the {@link BookEntry#COLUMN_BOOK_SUPNAME} key is present,
        // check that the supplier name value is not null.
        if (values.containsKey(BookEntry.COLUMN_BOOK_SUPNAME)) {
            String supname = values.getAsString(BookEntry.COLUMN_BOOK_SUPNAME);
            if (supname == null) {
                throw new IllegalArgumentException(String.valueOf(R.string.book_requires_supname));
            }
        ...
        // If the {@link BookEntry#COLUMN_BOOK_SUPPHONE} key is present,
        // check that the supplier phone number value is not null.
        if (values.containsKey(BookEntry.COLUMN_BOOK_SUPPHONE)) {
            String supphone = values.getAsString(BookEntry.COLUMN_BOOK_SUPPHONE);
            if (supphone == null) {
                throw new IllegalArgumentException(String.valueOf(R.string.book_requires_supphone));
            }
        }
...
/**Awesome
 All the CRUD operations work perfectly*/
